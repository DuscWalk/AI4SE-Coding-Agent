...
**容器 secret 文件**：设置 `CODING_AGENT_API_KEY_FILE` 指向只读挂载的 secret 文件。程序只在运行时读取文件，不把 key 写入镜像、配置或日志。

**支持的 LLM 供应商**：任何兼容 OpenAI API 协议的服务均可使用（OpenAI、Claude API、DeepSeek 等），通过 `OPENAI_BASE_URL` 环境变量或 `config.yaml` 中的 `model` 配置指定。

## 测试

一键运行全部质量门禁（ruff、mypy strict、全部测试）：

```bash
make check
```

Windows PowerShell 没有 `make` 时，可依次运行：
...
printf '%s' '你的 API key' > coding-agent-api-key
chmod 600 coding-agent-api-key
docker run --rm -p 8080:8080 \
  -v "$PWD/coding-agent-api-key:/run/secrets/coding-agent-api-key:ro" \
  -e CODING_AGENT_API_KEY_FILE=/run/secrets/coding-agent-api-key \
  coding-agent
```

Windows PowerShell 可先用 `Set-Content -NoNewline coding-agent-api-key '<API key>'` 创建文件，再将 `$PWD` 替换为 `${PWD}`。不要把 secret 文件加入 Git。

### 本地 Python 包

```bash
python -m build
pip install dist/coding_agent_harness-0.1.0-py3-none-any.whl
coding-agent serve --port 8080
```
...
| **路径** | `/opt/coding-agent/` |
| **凭据后端** | Linux 主机须配置 Secret Service；Docker 使用只读 secret 文件挂载，不使用明文 fallback keyring |
| **安全组** | 开放 8081 端口 |

### CI/CD

CI 配置位于 `.gitlab-ci.yml`，包含课程要求的 `unit-test` job；每次 push 自动运行全部测试、ruff、mypy strict，并构建 Docker 镜像。GitHub Actions 同步执行相同质量门禁。
...
- API key 默认存储在操作系统原生凭据管理器（Windows Credential Manager / macOS Keychain / Linux Secret Service），加密保护
- Docker 通过 `CODING_AGENT_API_KEY_FILE` 读取只读挂载的 secret，不把 key 写入镜像
