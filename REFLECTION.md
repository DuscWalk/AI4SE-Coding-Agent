...
所有核心机制测试都使用 mock LLM，完全确定性和可重复。115 个测试全部通过，无需真实 API key。
...
随着 task 数量增加，不同 agent 实现的接口之间出现了不一致，暴露了 PLAN.md 在接口定义上需要更精确。115 个测试全部使用 mock LLM 确保了确定性，但缺少真实 LLM 行为覆盖是一个已知风险。
...
本次项目通过 Superpowers 工作流完成了从零到交付的完整 Coding Agent Harness 实现。完整 commit 历史、115 个测试、3 个机制演示、完整的 WebUI 和 Docker 分发。
