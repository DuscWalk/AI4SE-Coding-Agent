...
### 7.1 Superpowers 技能：哪些发挥了最大作用，哪些形式大于实质

**发挥最大作用的技能**：

- `brainstorming`：8 轮结构化追问将"做一个 coding agent"这种模糊想法转化为可审查的设计规约。如果没有这个技能，我可能会跳过威胁模型、凭据生命周期、HITL 超时这些关键设计决策。
- `subagent-driven-development`：26 个 task 由 ~20 个独立 subagent 完成，每个都是全新上下文。这解决了 LLM 上下文中"幻觉累积"的问题——每个 agent 不会受前一个 task 的对话历史污染。
- `test-driven-development`：在 AI 协作场景中，TDD 是唯一客观的"绿灯"信号。subagent 声称"完成了"是不可信的，只有测试通过才是可信的。

**形式大于实质的技能**：

- `using-git-worktrees`：在本项目中实际上没有使用。26 个 task 中真正可并行的不超过 5 个，且 task 间有大量线性依赖。worktree 的文件系统隔离在此场景下增量收益不大，反而增加了分支管理的 overhead。
- `requesting-code-review`：两阶段评审（spec 合规 + 代码质量）是必要的，但作为独立技能调用增加了流程摩擦。实际上评审更自然地发生在主对话中——阅读 subagent 输出、运行测试、检查 commit diff。

### 7.2 最有效的 prompt/context 策略

最有效的策略是**给 subagent 提供完整的 task 上下文，而不是假设它知道项目背景**。每个 subagent 收到的 prompt 包含：
1. 具体文件路径（不是"在合适的地方添加"）
2. 完整的代码示例（不是"实现类似的功能"）
3. 精确的验证命令和预期输出（不是"运行测试确认通过"）

这对应了冷启动验证的核心教训：陌生 agent 会在你未明文写下的每个假设处受阻。好的 prompt 不是"写得聪明"，而是"写得完整"——让 agent 不需要猜测任何东西。

另一个有效策略是**用 mock LLM 消除不确定性**。在机制演示和单元测试中，ScriptedMockLLM 预设了完整的对话序列，让 agent 的行为完全可预测、可复现。这不仅让测试变得确定，也让调试变得可控——你知道每一步 LLM 会返回什么。

### 7.3 凭据与分发迫使你想清楚了什么

凭据安全让我意识到：**"安全"不是一个功能，而是一组必须在设计阶段就做出的决策**。keyring 选型、威胁模型、脱敏策略、首次运行引导——这些如果留到实现阶段再想，必然会出现遗漏。SPEC.md 的凭据威胁模型表格（§4.2）是整个项目中最有价值的工程产出之一——它迫使我在写代码之前就想清楚"key 可能从哪里泄露"。

分发（Docker + 公网部署）迫使我想清楚了**环境一致性**的问题。Windows 开发环境与 Linux 部署环境的差异（`mkdir -p` vs `New-Item`、keyring vs keyrings.alt、`python` vs `python3`）在冷启动验证和服务器部署中两次暴露，每次都是可避免的阻塞。如果重做，我会在 PLAN.md 中为每个 task 标注"在 Windows 和 Linux 上都验证过"。

### 7.4 对 Superpowers 方法论的批判

Superpowers 的核心假设是：**工程纪律（TDD、评审、计划、冷启动验证）可以用结构化技能的形式"嵌入"到 AI 协作流程中，从而减少人类工程师的纪律负担**。这个假设在本项目中基本成立。

但 Superpowers 也有其局限：

1. **假设 subagent 能独立完成一个 task**：实际上 subagent 经常因为接口不一致、上下文缺失或 plan 模糊而产生偏差。两阶段评审是必需的补救措施，但评审本身也需要人工判断力——AI 不能替代人类判断"这个实现是否正确"。

2. **假设 plan 可以事先写清楚**：冷启动验证暴露了 plan 中的事实性错误，但更多的问题是在实现过程中才暴露的——接口签名不一致、组件间的隐式依赖、行为边界不清晰。这些"未知的未知"很难在 plan 阶段预见。

3. **假设流程纪律可以弥补领域知识的缺失**：Superpowers 给了一套流程脚手架，但不会告诉你"反馈管线应该按速度排序"或"硬拦截模式不可 HITL 绕过"。这些设计决策仍然需要人类的领域判断。

总的来说，Superpowers 是一套有价值的"工程纪律外骨骼"——它提醒你在 AI 协作中不要跳过 TDD、不要跳过评审、不要跳过计划。但它不能替代工程判断力。最终，"做什么"和"做对了吗"仍然需要人来回答。
